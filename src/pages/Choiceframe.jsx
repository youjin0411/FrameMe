...
function Choiceframe() {
  ...
  const getBackgroundImage = () => {
    if (selectedFrame === 'f1') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew1));
      return `url(${Framew1})`;
    }
    if (selectedFrame === 'f2') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew2));
      return `url(${Framew2})`;
    }
    if (selectedFrame === 'f3') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew3));
      return `url(${Framew3})`;
    }
    if (selectedFrame === 'f4') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew4));
      return `url(${Framew4})`;
    }
    if (selectedFrame === 'f5') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew5));
      return `url(${Framew5})`;
    }
    if (selectedFrame === 'f6') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew6));
      return `url(${Framew6})`;
    }
    if (selectedFrame === 'f7') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew7));
      return `url(${Framew7})`;
    }
    if (selectedFrame === 'f8') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew8));
      return `url(${Framew8})`;
    }
    if (selectedFrame === 'f9') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew9));
      return `url(${Framew9})`;
    }
    if (selectedFrame === 'f10') {
      localStorage.setItem('selectedImages', JSON.stringify(Framew10));
      return `url(${Framew10})`;
    }
  };
  ...
  return (
    <div>
      <h1>프레임을 선택해주세요</h1>
      <div id="frameimg" style={frameImgStyle}>
        <div id="photoboxh1"></div>
        <div id="photoboxh2"></div>
        <div id="photoboxh3"></div>
      </div>
      <div style={{position: 'absolute', display: 'grid',left: 268, top: 290, gridRowGap: 10, rowGap: 10}}>
      <div style={{ ...style2, backgroundImage: `url(${storedImages[0]})` }} />
      <div style={{ ...style2, backgroundImage: `url(${storedImages[1]})` }} />
